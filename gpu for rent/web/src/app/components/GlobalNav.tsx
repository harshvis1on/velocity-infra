...
export default function GlobalNav({ userEmail }: { userEmail?: string }) {
  ...
  return (
    <nav className="border-b border-white/[0.06] bg-[#060606]/80 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          {userEmail && (
            <Link
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-1.5 ${
                isActive('/invite') ? 'text-yellow-400 bg-yellow-400/[0.06]' : 'text-gray-500 hover:text-yellow-400'
              }`}
            >
        ...
        <div className="flex items-center gap-3">
          {!userEmail ? (
            <>
              <Link href="/login" className="text-sm font-medium text-gray-500 hover:text-white transition-colors">Log in</Link>
              <Link href="/signup" className="bg-primary hover:bg-primary/90 text-black text-sm font-bold py-1.5 px-4 rounded-lg transition-all active:scale-[0.97]">
                Get Started Free
              </Link>
            </>
          ) : (
            <div className="flex items-center gap-3">
              ...
              <form action="/auth/signout" method="post">
                <button className="text-sm font-medium text-gray-500 hover:text-white transition-colors">Sign Out</button>
              </form>
              <Link href="/console" className="bg-primary hover:bg-primary/90 text-black text-sm font-bold py-1.5 px-4 rounded-lg transition-all active:scale-[0.97]">
                Console
              </Link>
            </div>
          )}
