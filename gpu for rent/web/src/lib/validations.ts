...
export const addMachineSchema = z.object({
  ...
  price_per_hour_usd: z.number().positive().max(10000),
...
export const createOfferSchema = z.object({
  ...
  pricePerGpuHrUsd: z.number().positive().max(50000),
  storagePricePerGbMonthUsd: z.number().min(0).max(500).default(4.5),
  bandwidthUploadPricePerGbUsd: z.number().min(0).max(100).default(0),
  bandwidthDownloadPricePerGbUsd: z.number().min(0).max(100).default(0),
  minGpu: z.number().int().min(1).max(16).refine(
    (v) => (v & (v - 1)) === 0,
    { message: 'min_gpu must be a power of 2 (1, 2, 4, 8, 16)' }
  ),
  offerEndDate: z.string().datetime(),
  interruptibleMinPriceUsd: z.number().positive().optional(),
  reservedDiscountFactor: z.number().min(0).max(1).default(0.4),
})

export const updateOfferSchema = z.object({
  pricePerGpuHrUsd: z.number().positive().max(50000).optional(),
  storagePricePerGbMonthUsd: z.number().min(0).max(500).optional(),
  bandwidthUploadPricePerGbUsd: z.number().min(0).max(100).optional(),
  bandwidthDownloadPricePerGbUsd: z.number().min(0).max(100).optional(),
  minGpu: z.number().int().min(1).max(16).refine(
    (v) => (v & (v - 1)) === 0,
    { message: 'min_gpu must be a power of 2' }
  ).optional(),
  offerEndDate: z.string().datetime().optional(),
  interruptibleMinPriceUsd: z.number().positive().nullable().optional(),
...
export const createRentalSchema = z.object({
  ...
  bidPriceUsd: z.number().positive().optional(),
...
export const addFundsSchema = z.object({
  amountUsd: z.number().positive().min(10).max(1000000),
